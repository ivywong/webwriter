...
export class CanvasComponent {
  ...
  private _pointerDownHandler(evt: PointerEvent) {
    ...
    } else if (this._isCardMenuAction("lock", target)) {
      console.log("lock card");
      const card = target.closest(".card-container") as HTMLDivElement;
      if (card.dataset.contentId) {
        if (card.classList.contains("locked")) {
          ...
          target
            ...
            ?.setAttribute("href", "#icon-unlocked");
          console.log(target);
        } else {
          card.classList.add("locked");
          target.querySelector("svg use")?.setAttribute("href", "#icon-locked");
          console.log(target);
        }
        this.store.toggleLockCard(card.dataset.contentId);
      }
